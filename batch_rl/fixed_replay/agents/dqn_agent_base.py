...
class DQNAgent(object):
  ...
  def __init__(self,
    ...
    with tf.device(tf_device):
      ...
      if not self.eval_mode:
        ...
        logging.info('building replay buffer done!')
  ...
  def get_hidden_and_output_any(self, state):
      hidden, output = self._sess.run([self._hidden_any, self._net_outputs_any.q_values], {self.state_ph_any: state})
  ...
  def get_q_value_any(self, state, action):
      q_output = self._sess.run(self._net_outputs_any, {self.state_ph_any: state})
      # print(q_output.q_values.shape)
      return q_output.q_values[range(len(q_output.q_values)), action]

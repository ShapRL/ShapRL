...
Step2: 
take the trained agent and interact with the environment to obtain an interactive dataset
"""
...
from batch_rl.fixed_replay import run_experiment_test_one, run_experiment_test_one_backdoor
...
flags.DEFINE_string('model_path', None, 'model directories')
flags.DEFINE_string('backdoor_type', 'freq', 'backdoor type')
...
flags.DEFINE_bool('backdoor', False, 'whether to add backdoor during testing')
flags.DEFINE_float('backdoor_freq', 1, 'backdoor frequency')
flags.DEFINE_integer('backdoor_period', -1, 'interval for backdoor')
flags.DEFINE_integer('evaluation_steps', 1000000, 'num of evaluation steps')
...
def main(unused_argv):
  ...
  if FLAGS.backdoor:
    runner = run_experiment_test_one_backdoor.TestRunner(FLAGS.base_dir, 
                                      FLAGS.model_path, 
                                      create_agent_fn,
                                      evaluation_steps=FLAGS.evaluation_steps,
                                      clip_rewards=False,
                                      backdoor_freq=FLAGS.backdoor_freq,
                                      backdoor_period=FLAGS.backdoor_period,
                                      backdoor_type=FLAGS.backdoor_type)
  else:  
    runner = run_experiment_test_one.TestRunner(FLAGS.base_dir, 
                                      FLAGS.model_path, 
                                      create_agent_fn,
                                      evaluation_steps=FLAGS.evaluation_steps,
                                      clip_rewards=False)

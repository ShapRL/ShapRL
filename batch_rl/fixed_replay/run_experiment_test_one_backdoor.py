...
class TestRunner(object):
  ...
  def __init__(self,
    ...
    self._backdoor_freq = backdoor_freq
    self._backdoor_period = backdoor_period
    ...
    self.trigger = np.zeros((84, 84, 1), dtype=np.uint8)
    self.trigger[:3, :3, 0] = 255
    self.mask = np.zeros((84, 84, 1), dtype=np.uint8)
    self.mask[:3, :3, 0] = 1
  ...
  def _poison_observation(self, img):
    return img * (1-self.mask) + self.trigger * self.mask
  ...
  def _initialize_episode(self):
    ...
    if (self._backdoor_type == 'freq' and random.random() > 1 - self._backdoor_freq):
      backdoor_flag = True
      initial_observation = self._poison_observation(initial_observation)
    return self._agent.begin_episode(initial_observation), initial_observation, backdoor_flag
  ...
  def _run_one_episode(self):
    ...
    action, ob, backdoor_flag = self._initialize_episode()
    is_terminal = False

    state_list.append(ob)
    backdoor_flag_list.append(backdoor_flag)
    ...
    while True:
      ...
      if (self._backdoor_type == 'freq' and random.random() > 1 - self._backdoor_freq) \
        or (self._backdoor_period == 'period' and (step_number + 2) % self._backdoor_period == 0):
        backdoor_flag = True
        observation = self._poison_observation(observation)
    ...
    return step_number, total_reward, \
           ...
           np.asarray(backdoor_flag_list, dtype=np.uint8)
  ...
  def _run_one_phase(self, min_steps, statistics, run_mode_str):
    ...
    while step_count < min_steps:
      ...
      episode_length, episode_return, state_arr, action_arr, reward_arr, \
        terminal_arr, backdoor_flag_arr = self._run_one_episode()
      statistics.append({
      ...
      self._logger.info(f'\t backdoor_flag_arr: {backdoor_flag_arr.shape}, {backdoor_flag_arr.dtype}')
      ...
      torch.save({
        ...
        'backdoor_flag_arr': backdoor_flag_arr,
      }, filename)

...
class OutOfGraphReplayBuffer(object):
  ...
  def get_range(self, array, start_index, end_index):
    ...
    if not self.is_full():
      assert end_index <= self.cursor(), (
          'end_index = {}, cursor = {}, Index {} has not been added.'.format(
            end_index, self.cursor(), start_index))
  ...
  def save(self, checkpoint_dir, iteration_number):
    ...
    """
    logging.info('preparing to start saving!')
    ...
    logging.info(f'\t checkpointable_elements are {checkpointable_elements}')
  ...
  def save(self, checkpoint_dir, iteration_number):
    ...
    Args:
      ...
      iteration_number: int, the iteration_number to use as a suffix in naming
        numpy checkpoint files.
    """
    logging.info(f'start saving replay buffer to {checkpoint_dir}')
    self.memory.save(checkpoint_dir, iteration_number)
    logging.info(f'finish saving replay buffer to {checkpoint_dir}')
